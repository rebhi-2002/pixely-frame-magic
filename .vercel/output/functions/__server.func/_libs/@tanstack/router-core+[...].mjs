import {
  ...
  B as rootRouteId,
  C as TSR_SCRIPT_BARRIER_ID,
  D as getStylesheetHref,
  F as dehydrateSsrMatchId,
  M as invariant,
  N as createLRUCache,
  P as decodePath,
  S as GLOBAL_TSR,
  T as createInlineCssStyleAsset,
  w as createInlineCssPlaceholderAsset,
...
async function parseObjectAsync(ctx, depth, id, current) {
  ...
  if (SYM_ITERATOR in current || SYM_ASYNC_ITERATOR in current)
    return parsePlainObject$1(ctx, depth, id, current, !!currentClass);
  throw new SerovalUnsupportedTypeError(current);
}
...
function serializeProperty(ctx, source, key, val) {
  if (typeof key === "string") {
    ...
    if (isValidKey(key))
      return (isIdentifier ? key : '"' + key + '"') + ":" + serialize$1(ctx, val);
...
export {
  ...
  toCrossJSONAsync as d,
  toCrossJSONStream as f,
  getNormalizedURL as i,
  makeSerovalPlugin as l,
  mergeHeaders as n,
  defaultSerovalPlugins as o,
  attachRouterServerSsrUtils as r,
  createRawStreamRPCPlugin as s,
  waitForRequest as t,
  fromJSON as u,
};
