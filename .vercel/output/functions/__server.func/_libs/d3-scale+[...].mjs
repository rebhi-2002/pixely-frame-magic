...
import {
  ...
  r as value_default,
  t as piecewise,
} from "./d3-interpolate.mjs";
import {
  a as formatPrefix,
  i as format,
  n as precisionPrefix_default,
  o as formatSpecifier,
  r as precisionFixed_default,
  t as precisionRound_default,
} from "./d3-format.mjs";
...
function transformer$2() {
  ...
  scale.invert = function (y) {
    return clamp(
      untransform((input || (input = piecewise(range, domain.map(transform), number_default)))(y)),
    );
  };
  scale.domain = function (_) {
...
function powish(transform) {
  ...
  function rescale() {
    return exponent === 1
      ? transform(identity$1, identity$1)
      : exponent === 0.5
...
function radial() {
  ...
  scale.range = function (_) {
    return arguments.length
      ? (squared.range((range = Array.from(_, number$1)).map(square)), scale)
      : range.slice();
  };
  scale.rangeRound = function (_) {
...
function timeInterval(floori, offseti, count, field) {
  function interval(date) {
    return (
      floori(
        (date =
          arguments.length === 0 ? /* @__PURE__ */ new Date() : /* @__PURE__ */ new Date(+date)),
      ),
      date
    );
  }
  ...
  interval.ceil = (date) => {
    return (
      floori((date = /* @__PURE__ */ new Date(date - 1))),
      offseti(date, 1),
      floori(date),
      date
    );
  ...
  interval.offset = (date, step) => {
    return (
      offseti((date = /* @__PURE__ */ new Date(+date)), step == null ? 1 : Math.floor(step)),
      date
    );
  };
  interval.range = (start, stop, step) => {
    ...
    let previous;
    do
      (range.push((previous = /* @__PURE__ */ new Date(+start))),
        offseti(start, step),
        floori(start));
...
function sequentialQuantile() {
  ...
  function scale(x) {
    if (x != null && !isNaN((x = +x)))
      return interpolator((bisectRight(domain, x, 1) - 1) / (domain.length - 1));
  }
  scale.domain = function (_) {
    if (!arguments.length) return domain.slice();
    domain = [];
...
export {
  ...
  band as T,
  radial as _,
  divergingSymlog as a,
  symlog as b,
  sequentialLog as c,
  sequentialSymlog as d,
  utcTime as f,
  quantile as g,
  quantize as h,
  divergingSqrt as i,
