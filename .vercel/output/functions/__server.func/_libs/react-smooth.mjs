...
var mapObject = function mapObject(fn, obj) {
  return Object.keys(obj).reduce(function (res, key) {
    return _objectSpread$2(
      _objectSpread$2({}, res),
      {},
      _defineProperty$2({}, key, fn(key, obj[key])),
    );
  }, {});
};
    ...
    switch (args[0]) {
      ...
      default:
        ...
        if (easing[0] === "cubic-bezier" && easing[1].split(")")[0].split(",").length === 4) {
          var _easing$1$split$0$spl2 = _slicedToArray$1(
            easing[1]
              .split(")")[0]
              .split(",")
              .map(function (x) {
...
var calStepperVals = function calStepperVals(easing, preVals, steps) {
  var nextStepVals = mapObject(function (key, val) {
    if (needContinue(val)) {
      var _easing2 = _slicedToArray(easing(val.from, val.to, val.velocity), 2),
        newX = _easing2[0],
        newV = _easing2[1];
...
var configUpdate_default = function (from, to, easing, duration, render) {
  ...
  var timingStyle = interKeys.reduce(function (res, key) {
    return _objectSpread$1(
      _objectSpread$1({}, res),
      {},
      _defineProperty$1({}, key, [from[key], to[key]]),
    );
  }, {});
  ...
  update = easing.isStepper
    ? function stepperUpdate(now) {
        if (!preTime) preTime = now;
        var steps = (now - preTime) / easing.dt;
        stepperStyle = calStepperVals(easing, stepperStyle, steps);
        render(
          _objectSpread$1(
            _objectSpread$1(_objectSpread$1({}, from), to),
  ...
  _createClass(Animate, [
    ...
    },
    {
      ...
      value: function runJSAnimation(props) {
        ...
        var startAnimation = configUpdate_default(
          from,
          to,
          configEasing(easing),
          duration,
    ...
    },
    {
      ...
      value: function runStepAnimation(props) {
        ...
        return this.manager.start(
          [onAnimationStart].concat(
            _toConsumableArray(
              steps.reduce(
                function addStyle(sequence, nextItem, index) {
                  ...
                  var newStyle = _objectSpread(
                    _objectSpread(_objectSpread({}, preItem.style), style),
                    {},
                    { transition },
                  );
                  return []
                    .concat(_toConsumableArray(sequence), [newStyle, duration, onAnimationEnd])
                    .filter(identity);
                },
                [initialStyle, Math.max(initialTime, begin)],
              ),
            ),
...
Animate.propTypes = {
  from: import_prop_types.default.oneOfType([
    import_prop_types.default.object,
    import_prop_types.default.string,
  ]),
  to: import_prop_types.default.oneOfType([
    import_prop_types.default.object,
    import_prop_types.default.string,
  ]),
  ...
  steps: import_prop_types.default.arrayOf(
