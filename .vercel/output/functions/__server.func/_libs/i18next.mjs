  ...
  if (path.length > 1 && nsSeparator) {
    ...
    const nsList = strict
      ...
      : Array.isArray(ns)
        ...
        : null;
    if (nsList) {
      if ((strict ? nsList : nsList.length > 1 ? nsList.slice(1) : []).includes(path[0]))
        return `${path[0]}${nsSeparator}${path.slice(1).join(keySeparator)}`;
    }
  }
  return path.join(keySeparator);
}
var shouldHandleAsObject = (res) =>
...
var LanguageUtil = class {
  ...
  toResolveHierarchy(code, fallbackCode) {
    ...
    const hasCacheableFallback =
      fallbackCode === void 0 || fallbackCode === false || isString(fallbackCode);
    const usesUncacheableOptionsFallback =
      fallbackCode === void 0 && typeof this.options.fallbackLng === "function";
