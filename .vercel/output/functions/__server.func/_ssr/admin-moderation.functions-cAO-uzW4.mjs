...
import { s as requirePermission, t as createServerRpc } from "./rbac.server-CL_Sz4LS.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/admin-moderation.functions-cAO-uzW4.js
...
var listTeacherVerifications = createServerFn({ method: "GET" })
  .middleware([requireAuth])
  .handler(listTeacherVerifications_createServerFn_handler, async ({ context }) => {
...
var saveTeacherVerification = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) =>
    objectType({
      ...
      status: enumType(["قيد المراجعة", "مكتمل", "ينقص مستند", "مرفوض"]),
      notes: stringType().trim().max(300).optional().or(literalType("")),
    }).parse(input),
  )
  .handler(saveTeacherVerification_createServerFn_handler, async ({ data, context }) => {
...
var deleteTeacherVerification = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) => objectType({ id: stringType() }).parse(input))
  .handler(deleteTeacherVerification_createServerFn_handler, async ({ data, context }) => {
...
var listContentSubmissions = createServerFn({ method: "GET" })
  .middleware([requireAuth])
  .handler(listContentSubmissions_createServerFn_handler, async ({ context }) => {
...
var saveContentSubmission = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) =>
    objectType({
      id: stringType().optional(),
      ...
      type: enumType(["درس", "اختبار", "كورس"]),
      status: enumType(["جديد", "مراجعة ثانية", "جاهز للاعتماد", "معتمد", "مرفوض"]),
    }).parse(input),
  )
  .handler(saveContentSubmission_createServerFn_handler, async ({ data, context }) => {
...
var deleteContentSubmission = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) => objectType({ id: stringType() }).parse(input))
  .handler(deleteContentSubmission_createServerFn_handler, async ({ data, context }) => {
...
var listCommunityReports = createServerFn({ method: "GET" })
  .middleware([requireAuth])
  .handler(listCommunityReports_createServerFn_handler, async ({ context }) => {
...
var saveCommunityReport = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) =>
    objectType({
      id: stringType().optional(),
      ...
      priority: enumType(["عالية", "متوسطة", "منخفضة"]),
      status: enumType(["مفتوح", "مغلق", "مؤجل"]),
    }).parse(input),
  )
  .handler(saveCommunityReport_createServerFn_handler, async ({ data, context }) => {
    await requirePermission(
      context.userId,
...
var deleteCommunityReport = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) => objectType({ id: stringType() }).parse(input))
  .handler(deleteCommunityReport_createServerFn_handler, async ({ data, context }) => {
...
//#endregion
export {
  deleteCommunityReport_createServerFn_handler,
  deleteContentSubmission_createServerFn_handler,
  deleteTeacherVerification_createServerFn_handler,
  listCommunityReports_createServerFn_handler,
  listContentSubmissions_createServerFn_handler,
  listTeacherVerifications_createServerFn_handler,
  saveCommunityReport_createServerFn_handler,
  saveContentSubmission_createServerFn_handler,
  saveTeacherVerification_createServerFn_handler,
};
