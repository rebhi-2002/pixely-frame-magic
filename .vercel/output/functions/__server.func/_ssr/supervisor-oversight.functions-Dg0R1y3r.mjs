...
import { t as LINKED_CHILDREN } from "./account-pages-data-DAenjrEL.mjs";
import { i as MISTAKES, r as EXAM_ATTEMPTS } from "./student-evaluation-data-z7p3-_Ty.mjs";
import { o as WEEKLY_STUDY_LOG, r as LIBRARY_SUBJECTS } from "./student-learning-data-C3xcWuN0.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/supervisor-oversight.functions-Dg0R1y3r.js
...
var listTeacherPerformance = createServerFn({ method: "GET" })
  .middleware([requireAuth])
  .handler(listTeacherPerformance_createServerFn_handler, async ({ context }) => {
...
var saveTeacherPerformance = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) =>
    objectType({
      ...
      status: enumType(["ممتاز", "جيد", "تأخر تصحيح"]),
    }).parse(input),
  )
  .handler(saveTeacherPerformance_createServerFn_handler, async ({ data, context }) => {
...
var deleteTeacherPerformance = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) => objectType({ id: stringType() }).parse(input))
  .handler(deleteTeacherPerformance_createServerFn_handler, async ({ data, context }) => {
...
var listStudentRisk = createServerFn({ method: "GET" })
  .middleware([requireAuth])
  .handler(listStudentRisk_createServerFn_handler, async ({ context }) => {
...
var saveStudentRisk = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) =>
    objectType({
      id: stringType().optional(),
      ...
      status: enumType(["متعثّر", "مراقبة", "منتظم"]),
    }).parse(input),
  )
  .handler(saveStudentRisk_createServerFn_handler, async ({ data, context }) => {
    await requirePageAction(
      context.userId,
      "supervisor_students",
      data.id ? "edit" : "execute_add",
...
var deleteStudentRisk = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) => objectType({ id: stringType() }).parse(input))
  .handler(deleteStudentRisk_createServerFn_handler, async ({ data, context }) => {
...
var listSupervisionReports = createServerFn({ method: "GET" })
  .middleware([requireAuth])
  .handler(listSupervisionReports_createServerFn_handler, async ({ context }) => {
...
var recordReportDownload = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) => objectType({ id: stringType() }).parse(input))
  .handler(recordReportDownload_createServerFn_handler, async ({ data, context }) => {
...
var deleteSupervisionReport = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((input) => objectType({ id: stringType() }).parse(input))
  .handler(deleteSupervisionReport_createServerFn_handler, async ({ data, context }) => {
...
var getSupervisionSettings = createServerFn({ method: "GET" })
  .middleware([requireAuth])
  .handler(getSupervisionSettings_createServerFn_handler, async ({ context }) => {
    await requirePageAction(context.userId, "supervisor_reports", "view_list");
...
var getChildReport = createServerFn({ method: "GET" })
  .middleware([requireAuth])
  .handler(getChildReport_createServerFn_handler, async ({ context }) => {
...
//#endregion
export {
  deleteStudentRisk_createServerFn_handler,
  deleteSupervisionReport_createServerFn_handler,
  deleteTeacherPerformance_createServerFn_handler,
  getChildReport_createServerFn_handler,
  getSupervisionSettings_createServerFn_handler,
  listStudentRisk_createServerFn_handler,
  listSupervisionReports_createServerFn_handler,
  listTeacherPerformance_createServerFn_handler,
  recordReportDownload_createServerFn_handler,
  saveStudentRisk_createServerFn_handler,
