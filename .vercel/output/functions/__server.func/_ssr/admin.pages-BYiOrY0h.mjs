...
import { D as useBi, d as apiClient, f as getErrorMessage } from "./rbac-static-data-C-KJ3jWh.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/admin.pages-BYiOrY0h.js
...
async function saveBackendPage(form) {
	if (!form.name.trim() || form.name.trim().length < 3) throw new Error("الاسم بالعربي مطلوب (3 أحرف على الأقل)");
	if (!form.name_en.trim() || form.name_en.trim().length < 3) throw new Error("الاسم بالإنجليزي مطلوب (3 أحرف على الأقل)");
	if (form.category_id == null) throw new Error("يجب اختيار الفئة");
	if (form.id != null && form.parent_id === form.id) throw new Error("لا يمكن أن تكون الصفحة أبًا لنفسها");
...
async function deleteBackendPage(id) {
	...
	if (!result.success) throw new Error(result.message || "تعذر حذف الصفحة");
}
var EMPTY_FORM = {
	name: "",
	name_en: "",
