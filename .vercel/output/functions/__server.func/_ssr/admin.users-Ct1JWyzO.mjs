...
//#region node_modules/.nitro/vite/services/ssr/assets/admin.users-Ct1JWyzO.js
...
async function saveBackendUser(form) {
	...
	if (!form.id && (!form.password || !form.confirmPassword)) throw new Error("كلمة المرور وتأكيدها مطلوبان عند إضافة مستخدم");
...
async function updateBackendUserStatus(id, isActive) {
	...
	if (!user?.id || !user.name || !user.email || !user.phoneNumber) throw new Error("تعذر تحميل بيانات المستخدم قبل تحديث حالته");
