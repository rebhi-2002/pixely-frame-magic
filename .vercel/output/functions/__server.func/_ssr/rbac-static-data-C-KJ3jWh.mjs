...
//#region node_modules/.nitro/vite/services/ssr/assets/rbac-static-data-C-KJ3jWh.js
...
function friendlyMessageFor(kind, status) {
	if (kind === "network") return "تعذّر الاتصال بالخادم. تأكد من اتصالك بالإنترنت وحاول مجددًا — إذا استمرت المشكلة، الخادم قد يكون متوقفًا مؤقتًا.";
	if (kind === "timeout") return "استغرق الطلب وقتًا أطول من المتوقع. حاول مرة أخرى.";
	if (kind === "parse") return "وصل ردّ غير متوقع من الخادم. حاول مجددًا، وإذا تكررت المشكلة بلّغ الدعم الفني.";
	switch (status) {
		case 400: return "البيانات المُرسلة غير صحيحة. راجع الحقول وحاول مجددًا.";
		case 401: return "انتهت جلستك أو لم يتم تسجيل الدخول. سجّل الدخول مجددًا للمتابعة.";
		case 403: return "ليس لديك صلاحية للقيام بهذا الإجراء.";
		case 404: return "لم يتم العثور على البيانات المطلوبة.";
		case 409: return "تعارض في البيانات — قد يكون هذا العنصر معدّلاً من مكان آخر. حدّث الصفحة وحاول مجددًا.";
		case 422: return "تعذّر معالجة البيانات المُرسلة. راجع الحقول وحاول مجددًا.";
		case 429: return "طلبات كثيرة خلال وقت قصير. انتظر قليلًا وحاول مجددًا.";
		default:
			if (status >= 500) return "حدث خطأ من جهة الخادم. حاول لاحقًا، وإذا استمرت المشكلة بلّغ الدعم الفني.";
...
//#endregion
export { pageMatchesRole as C, useBi as D, roleKeyFromName as E, usePreferences as O, nextId as S, roleHome as T, isAuthenticated as _, PERMISSION_KEYS as a, login as b, ROLE_PERMISSION_GRANTS as c, apiClient as d, getErrorMessage as f, i18n_default as g, getStoredUserId as h, PAGES as i, verifyServerSession as k, USERS as l, getStoredProfile as m, DEMO_USER_COOKIE as n, PreferencesProvider as o, getStoredEmail as p, MODULES as r, ROLES as s, AUTH_EVENT as t, allowedPublicPaths as u, isDemoSession as v, preferencesBootScript as w, logout as x, isRealAdmin as y };
