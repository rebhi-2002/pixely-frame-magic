...
import { C as nextId, c as ROLE_PERMISSION_GRANTS, l as USERS, r as MODULES, s as ROLES } from "./rbac-static-data-DgiM51a_.mjs";
import { r as createServerFn } from "./server-CQPo-kzR.mjs";
import { t as requireAuth } from "./auth-middleware-DsGFXEB-.mjs";
import { a as requireAdmin, i as loadUsers, n as loadAccess, o as requirePageAction, r as loadPermissionMatrix, s as requirePermission, t as createServerRpc } from "./rbac.server-CmWGNCpO.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/rbac.functions-DtHh5qVP.js
...
var getMyAccess = createServerFn({ method: "GET" }).middleware([requireAuth]).handler(getMyAccess_createServerFn_handler, async ({ context }) => loadAccess(context.userId));
...
var listModules = createServerFn({ method: "GET" }).middleware([requireAuth]).handler(listModules_createServerFn_handler, async ({ context }) => {
	await requirePermission(context.userId, "admin_settings", "view_list");
	return MODULES.slice().sort((a, b) => a.sort_order - b.sort_order).map((m) => ({ ...m }));
});
...
var setModuleEnabled = createServerFn({ method: "POST" }).middleware([requireAuth]).inputValidator((input) => objectType({
	id: stringType(),
	enabled: booleanType()
}).parse(input)).handler(setModuleEnabled_createServerFn_handler, async ({ data, context }) => {
...
var listRoles = createServerFn({ method: "GET" }).middleware([requireAuth]).handler(listRoles_createServerFn_handler, async ({ context }) => {
...
}).parse(input)).handler(saveRole_createServerFn_handler, async ({ data, context }) => {
...
var deleteRole = createServerFn({ method: "POST" }).middleware([requireAuth]).inputValidator((input) => objectType({ id: stringType() }).parse(input)).handler(deleteRole_createServerFn_handler, async ({ data, context }) => {
...
var getPermissionMatrix = createServerFn({ method: "POST" }).middleware([requireAuth]).inputValidator((input) => objectType({ roleId: stringType() }).parse(input)).handler(getPermissionMatrix_createServerFn_handler, async ({ data, context }) => {
	await requirePermission(context.userId, "admin_roles", "view_list");
...
}).parse(input)).handler(saveRolePermissions_createServerFn_handler, async ({ data, context }) => {
	await requireAdmin(context.userId);
	const role = ROLES.find((r) => r.id === data.roleId);
	if (!role) throw new Error("نوع المستخدم غير موجود");
	if (role.name === "مدير عام") return {
		ok: true,
...
var listUsers = createServerFn({ method: "GET" }).middleware([requireAuth]).handler(listUsers_createServerFn_handler, async ({ context }) => {
...
}).parse(input)).handler(saveUser_createServerFn_handler, async ({ data, context }) => {
...
}).parse(input)).handler(toggleUserStatus_createServerFn_handler, async ({ data, context }) => {
...
var deleteUser = createServerFn({ method: "POST" }).middleware([requireAuth]).inputValidator((input) => objectType({ id: stringType() }).parse(input)).handler(deleteUser_createServerFn_handler, async ({ data, context }) => {
...
}).parse(input)).handler(sendPasswordReset_createServerFn_handler, async ({ data, context }) => {
	await requirePermission(context.userId, "admin_users", "change_password");
	if (!USERS.find((u) => u.id === data.id)?.email) throw new Error("لا يوجد بريد إلكتروني لهذا المستخدم");
...
var updateOwnProfile = createServerFn({ method: "POST" }).middleware([requireAuth]).inputValidator((input) => objectType({
	full_name: stringType().trim().min(2, "الاسم قصير جداً"),
	email: stringType().trim().email("بريد غير صالح").optional().or(literalType(""))
}).parse(input)).handler(updateOwnProfile_createServerFn_handler, async ({ data, context }) => {
...
//#endregion
export { deleteRole_createServerFn_handler, deleteUser_createServerFn_handler, getMyAccess_createServerFn_handler, getPermissionMatrix_createServerFn_handler, listModules_createServerFn_handler, listRoles_createServerFn_handler, listUsers_createServerFn_handler, saveRolePermissions_createServerFn_handler, saveRole_createServerFn_handler, saveUser_createServerFn_handler, sendPasswordReset_createServerFn_handler, setModuleEnabled_createServerFn_handler, toggleUserStatus_createServerFn_handler, updateOwnProfile_createServerFn_handler };
