...
export function AdminDashboardPage() {
  ...
  // 1. استدعاء جميع الـ Hooks دائماً وبنفس الترتيب قبل أي شرط أو return
  const verifications = useMemo(() => q1.data ?? [], [q1.data]);
  const content = useMemo(() => q2.data ?? [], [q2.data]);
  const reports = useMemo(() => q3.data ?? [], [q3.data]);
  const payoutRows = useMemo(() => q4.data ?? [], [q4.data]);
  const subjects = useMemo(() => q5.data ?? [], [q5.data]);
  const requests = useMemo(() => q6.data ?? [], [q6.data]);
  const allUsers = useMemo(() => q7.data ?? [], [q7.data]);
